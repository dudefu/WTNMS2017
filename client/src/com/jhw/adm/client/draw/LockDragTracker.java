...
public class LockDragTracker extends AbstractTool implements DragTracker {

	public LockDragTracker(Figure figure) {
		anchorFigure = figure;
	}

	public LockDragTracker() {
	}
	...
	protected void maybeFireBoundsInvalidated(Rectangle invalidatedArea) {
		...
		if (!canvasBounds.contains(invalidatedArea)) {
			...
			if (anchorFigure != null) {
				// 滚动到最新的位置
				Rectangle invalidated = new Rectangle((int)anchorFigure.getBounds().x, (int)anchorFigure.getBounds().y, 0, 0);
	            invalidated.add(anchorFigure.getDrawingArea());
