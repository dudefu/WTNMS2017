...
@Component(CommonAlarmConfirmView.ID)
@Scope(Scopes.DESKTOP)
public class CommonAlarmConfirmView extends ViewPart {

	private static final long serialVersionUID = -1288837887099756518L;
	public static final String ID = "commonAlarmConfirmView";
	private static final Logger LOG = LoggerFactory.getLogger(CommonAlarmConfirmView.class);
	...
	private static final String MESSAGE = "普通告警总数：%s,选中告警数：%s";
	...
	private JCommonAlarmButtonModel commonModel;
	...
	private PropertyChangeListener commonAlarmListener = new PropertyChangeListener() {
		...
		public void propertyChange(PropertyChangeEvent evt) {
			updateCommonAlarmTableData();
		}
	};
	
	private PropertyChangeListener commonAlarmUpdateAfterConfirmListener = new PropertyChangeListener() {
		@Override
		public void propertyChange(PropertyChangeEvent evt) {
			setTableData();
		}
	...
	protected void initialize(){
		...
		commonModel.addPropertyChangeListener(JCommonAlarmButtonModel.UPDATE_COMMON_VIEW, commonAlarmListener);
		commonModel.addPropertyChangeListener(JCommonAlarmButtonModel.NOTICE_VIEW_UPDATE_AFTER_CONFIRM_COMMON, commonAlarmUpdateAfterConfirmListener);
		...
		this.setTitle("普通告警列表");
		
		initializeTableData();
	}

	protected void updateCommonAlarmTableData() {
		if(isSelected == true){
			setTableData();
	...
	private void setTableData() {
		if(SwingUtilities.isEventDispatchThread()){
			model.setWarningAlarms(commonModel.getCommonAlam());
			model.fireTableDataChanged();
			setMEssageLabel(commonModel.getCommonAlam().size(), 0);
...
//		table.setColumnControlVisible(true);
		...
		table.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				setMEssageLabel(commonModel.getCommonAlam().size(), table.getSelectedRowCount());
				selectOneRow();
			}
		});
	...
	@ViewAction(name=CONFIRM, icon=ButtonConstants.SAVE, desc="确认普通告警信息",role=Constants.MANAGERCODE)
	public void confirm(){
	...
	private class RequestTask implements Task{
		...
		public void run() {
			...
			try{
				...
				strategy.showErrorMessage("确认普通告警异常");
	...
	private void showMessageDialog(final Task task){
		if(SwingUtilities.isEventDispatchThread()){
			...
			strategy = new WarningConfirmStrategy("确认普通告警", progressBarModel);
			dialog = new JProgressBarDialog("提示",0,1,this,"确认普通告警",true);
	...
	@ViewAction(name=EXPORT, icon=ButtonConstants.EXPORT, desc="导出普通告警信息",role=Constants.MANAGERCODE)
	public void export(){
		List<WarningEntity> warninglist = commonModel.getCommonAlam();
		...
		if(FileImportAndExport.SUCCESS_RESULT == result){
			JOptionPane.showMessageDialog(this, "普通告警信息导出成功", "提示", JOptionPane.INFORMATION_MESSAGE);
			...
			JOptionPane.showMessageDialog(this, "普通告警信息导出失败", "提示", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public void dispose() {
		super.dispose();
		commonModel.removePropertyChangeListener(JCommonAlarmButtonModel.UPDATE_COMMON_VIEW, commonAlarmListener);
		commonModel.removePropertyChangeListener(JCommonAlarmButtonModel.NOTICE_VIEW_UPDATE_AFTER_CONFIRM_COMMON, commonAlarmUpdateAfterConfirmListener);
	}
