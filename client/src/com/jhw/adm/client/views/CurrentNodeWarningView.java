...
public class CurrentNodeWarningView extends ViewPart {
	...
	private void createContents(JPanel parent) {
		...
		scrollTablePanel.setBorder(BorderFactory.createTitledBorder("告警信息"));
	...
	@ViewAction(name=QUERY, icon=ButtonConstants.QUERY, desc="查询当前节点告警信息",role=Constants.MANAGERCODE)
	public void query() {
	...
	@ViewAction(name=EXPORT, icon=ButtonConstants.EXPORT, desc="导出当前节点告警信息",role=Constants.MANAGERCODE)
	public void export(){
		Date startDate = fromPicker.getDate();
		Date endDate = thruPicker.getDate();
		int level = ((StringInteger)levelCombox.getSelectedItem()).getValue();
