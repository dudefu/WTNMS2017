...
@Component(MemoryMonitorView.ID)
public class MemoryMonitorView extends ViewPart {
	...
	boolean doControls;
	JTextField tf;
	public static final String ID = "memoryMonitorView";
	// Get memory pools.
...
//		new Thread(new Memeater()).start();
	...
	public class Surface extends JPanel implements Runnable {
		...
		public void plotMemoryUsage(int x1, int y1, int x2, int y2, int npool) {
			...
			int i = 0;
			for (; i < MemUsage; i++) {
				mfRect.setRect(x1 + 5, y1 + ssH + i * blockHeight, blockWidth,
						blockHeight - 1);
				big.fill(mfRect);
			}
			...
			big.setColor(Color.green);
			for (; i < 10; i++) {
				muRect.setRect(x1 + 5, y1 + ssH + i * blockHeight, blockWidth,
						blockHeight - 1);
				big.fill(muRect);
			}
		...
		public void run() {
			...
			while (thread == me && isShowing()) {
				...
				if (MemoryMonitorView.dateStampCB.isSelected()) {
