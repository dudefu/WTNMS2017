...
@Component(NoticeAlarmConfirmView.ID)
@Scope(Scopes.DESKTOP)
public class NoticeAlarmConfirmView extends ViewPart {

	private static final long serialVersionUID = -1288837887099756518L;
	public static final String ID = "noticeAlarmConfirmView";
	private static final Logger LOG = LoggerFactory.getLogger(NoticeAlarmConfirmView.class);
	...
	private static final String MESSAGE = "通知告警总数：%s,选中告警数：%s";
	...
	private JNoticeAlarmButtonModel noticeModel;
	...
	private PropertyChangeListener noticeAlarmListener = new PropertyChangeListener() {
		...
		public void propertyChange(PropertyChangeEvent evt) {
			updateNoticeAlarmTableData();
		}
	};
	
	private PropertyChangeListener noticeAlarmUpdateAfterConfirmListener = new PropertyChangeListener() {
		@Override
		public void propertyChange(PropertyChangeEvent evt) {
			setTableData();
		}
	...
	protected void initialize(){
		...
		noticeModel.addPropertyChangeListener(JNoticeAlarmButtonModel.UPDATE_NOTICE_VIEW, noticeAlarmListener);
		noticeModel.addPropertyChangeListener(JNoticeAlarmButtonModel.NOTICE_VIEW_UPDATE_AFTER_CONFIRM_NOTICE, noticeAlarmUpdateAfterConfirmListener);
		...
		this.setTitle("通知告警列表");
		
		initializeTableData();
	}

	protected void updateNoticeAlarmTableData() {
		if(isSelected == true){
			setTableData();
	...
	private void setTableData() {
		if(SwingUtilities.isEventDispatchThread()){
			model.setWarningAlarms(noticeModel.getNoticeAlam());
			model.fireTableDataChanged();
			setMessageLabel(noticeModel.getNoticeAlam().size(), 0);
	...
	private void createTablePanel(JScrollPane parent) {
		...
		table.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				setMessageLabel(noticeModel.getNoticeAlam().size(), table.getSelectedRowCount());
				selectOneRow();
			}
		});
	...
	@ViewAction(name=CONFIRM, icon=ButtonConstants.SAVE, desc="确认通知告警信息",role=Constants.MANAGERCODE)
	public void confirm(){
	...
	private class RequestTask implements Task{
		...
		public void run() {
			...
			try{
				...
				strategy.showErrorMessage("确认通知告警异常");
				LOG.error("Error occur when confirming notice warning(s)", e);
			}
	...
	private void showMessageDialog(final Task task){
		if(SwingUtilities.isEventDispatchThread()){
			...
			strategy = new WarningConfirmStrategy("确认通知告警", progressBarModel);
			dialog = new JProgressBarDialog("提示",0,1,this,"确认通知告警",true);
	...
	@ViewAction(name=EXPORT, icon=ButtonConstants.EXPORT, desc="导出通知告警信息",role=Constants.MANAGERCODE)
	public void export(){
		List<WarningEntity> warninglist = noticeModel.getNoticeAlam();
		...
		if(FileImportAndExport.SUCCESS_RESULT == result){
			JOptionPane.showMessageDialog(this, "通知告警信息导出成功", "提示", JOptionPane.INFORMATION_MESSAGE);
			...
			JOptionPane.showMessageDialog(this, "通知告警信息导出失败", "提示", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public void dispose() {
		super.dispose();
		noticeModel.removePropertyChangeListener(JNoticeAlarmButtonModel.UPDATE_NOTICE_VIEW, noticeAlarmListener);
		noticeModel.removePropertyChangeListener(JNoticeAlarmButtonModel.NOTICE_VIEW_UPDATE_AFTER_CONFIRM_NOTICE, noticeAlarmUpdateAfterConfirmListener);
	}
