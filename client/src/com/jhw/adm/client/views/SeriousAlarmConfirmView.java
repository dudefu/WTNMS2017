...
@Component(SeriousAlarmConfirmView.ID)
@Scope(Scopes.DESKTOP)
public class SeriousAlarmConfirmView extends ViewPart {

	private static final long serialVersionUID = -1288837887099756518L;
	public static final String ID = "seriousAlarmConfirmView";
	private static final Logger LOG = LoggerFactory.getLogger(SeriousAlarmConfirmView.class);
	...
	private static final String MESSAGE = "严重告警总数：%s,选中告警数：%s";
	...
	private JSeriousAlarmButtonModel seriousModel;
	...
	private PropertyChangeListener seriousAlarmListener = new PropertyChangeListener() {
		...
		public void propertyChange(PropertyChangeEvent evt) {
			updateSeriousAlarmTableData();
		}
	};
	
	private PropertyChangeListener seriousAlarmUpdateAfterConfirmListener = new PropertyChangeListener() {
		@Override
		public void propertyChange(PropertyChangeEvent evt) {
			setTableData();
		}
	...
	protected void initialize(){
		...
		seriousModel.addPropertyChangeListener(JSeriousAlarmButtonModel.UPDATE_SERIOUS_VIEW, seriousAlarmListener);
		seriousModel.addPropertyChangeListener(JSeriousAlarmButtonModel.NOTICE_VIEW_UPDATE_AFTER_CONFIRM_SERIOUS, seriousAlarmUpdateAfterConfirmListener);
		...
		this.setTitle("严重告警列表");
		
		initializeTableData();
	}

	protected void updateSeriousAlarmTableData() {
		if(isSelected == true){
			setTableData();
	...
	private void setTableData() {
		if(SwingUtilities.isEventDispatchThread()){
			model.setWarningAlarms(seriousModel.getSeriousAlam());
			model.fireTableDataChanged();
			setMEssageLabel(seriousModel.getSeriousAlam().size(), 0);
...
//		table.setColumnControlVisible(true);
		...
		table.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				setMEssageLabel(seriousModel.getSeriousAlam().size(), table.getSelectedRowCount());
				selectOneRow();
			}
		});
	...
	@ViewAction(name=CONFIRM, icon=ButtonConstants.SAVE, desc="确认严重告警信息",role=Constants.MANAGERCODE)
	public void confirm(){
	...
	private class RequestTask implements Task{
		...
		public void run() {
			...
			try{
				...
				strategy.showErrorMessage("确认严重告警异常");
				LOG.error("Error occur when confirming serious warning(s)", e);
			}
	...
	private void showMessageDialog(final Task task){
		if(SwingUtilities.isEventDispatchThread()){
			...
			strategy = new WarningConfirmStrategy("确认严重告警", progressBarModel);
			dialog = new JProgressBarDialog("提示",0,1,this,"确认严重告警",true);
	...
	@ViewAction(name=EXPORT, icon=ButtonConstants.EXPORT, desc="导出严重告警信息",role=Constants.MANAGERCODE)
	public void export(){
		List<WarningEntity> warninglist = seriousModel.getSeriousAlam();
		...
		if(FileImportAndExport.SUCCESS_RESULT == result){
			JOptionPane.showMessageDialog(this, "严重告警信息导出成功", "提示", JOptionPane.INFORMATION_MESSAGE);
			...
			JOptionPane.showMessageDialog(this, "严重告警信息导出失败", "提示", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public void dispose() {
		super.dispose();
		seriousModel.removePropertyChangeListener(JSeriousAlarmButtonModel.UPDATE_SERIOUS_VIEW, seriousAlarmListener);
		seriousModel.removePropertyChangeListener(JSeriousAlarmButtonModel.NOTICE_VIEW_UPDATE_AFTER_CONFIRM_SERIOUS, seriousAlarmUpdateAfterConfirmListener);
	}
