...
public class ICMPHeader extends Object {
	 ...
	 * 
	 */
	public ICMPHeader(byte type, byte code, short checksum, short identity,
			short sequence) {
		m_type = type;
		m_code = code;
		m_checksum = checksum;
	...
	public ICMPHeader(byte[] data, int offset) {
		if ((data.length - offset) < getNetworkSize())
			throw new IndexOutOfBoundsException(
					"Insufficient number of bytes available to construct the ICMP header");
	...
	protected int storeToBuffer(byte[] buf, int offset) {
		if (buf.length < (offset + 8))
			throw new IndexOutOfBoundsException(
					"Array index overflow in buffer");
	...
	protected int loadFromBuffer(byte[] buf, int offset) {
		if (buf.length < (offset + 8))
			throw new IndexOutOfBoundsException(
					"Insufficient data to load ICMP header");
