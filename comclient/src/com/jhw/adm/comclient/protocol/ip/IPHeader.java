...
public class IPHeader extends Object {
	...
	public static class RouteOption extends Option {
		...
		void add(IPv4Address addr) {
			if (m_addrs.size() == 9)
				throw new IndexOutOfBoundsException(
						"The address could not be added, the record is full");
			m_addrs.add(addr);
		}
		...
		RouteOption(byte code, IPv4Address[] addrs) {
			...
			if (addrs.length > 9)
				throw new IndexOutOfBoundsException(
						"Route Option List Cannot Exceed 9 Addresses");
	...
	public IPHeader(byte[] header, int offset) {
		...
		//
		if (m_version != 4)
			throw new UnknownIPVersionException(
					"Unknown IP Version, version = " + m_version);
		...
		if ((length - offset) < (m_hdrlen * 4))
			throw new IndexOutOfBoundsException(
					"Insufficient data: buffer size = " + (length - offset)
							+ " and header length = " + (m_hdrlen * 4));
		...
		// in bytes.
		//
		m_length = (short) (byteToShort(header[offset + ndx]) << 8 | byteToShort(header[offset
				+ ndx + 1]));
		ndx += 2;
		...
		//
		m_identity = (short) (byteToShort(header[offset + ndx]) << 8 | byteToShort(header[offset
				+ ndx + 1]));
		ndx += 2;
		...
		// fragment offset!
		//
		m_fragOffset = (short) (byteToShort(header[offset + ndx]) << 8 | byteToShort(header[offset
				+ ndx + 1]));
		...
		//
		m_checksum = (short) (byteToShort(header[offset + ndx]) << 8 | byteToShort(header[offset
				+ ndx + 1]));
		ndx += 2;
	...
	public void addOption(Option opt) {
		...
		if (m_options == null) {
			...
			if (origLen + opt.bytesRequired() > 40)
				throw new IndexOutOfBoundsException(
						"Option List is too long, must be less than 40 bytes");
